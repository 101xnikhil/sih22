    ...
    <header className="h-20 bg-transparent flex items-center justify-between px-4 sm:px-6 lg:px-8 z-30 transition-all max-w-[1600px] w-full mx-auto">
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className="lg:hidden text-slate-600 hover:text-slate-900 focus:outline-none p-2 rounded-xl bg-white border border-[#e5e9f2] shadow-sm"
          aria-label="Toggle navigation menu"
        >
          <Menu className="w-5 h-5" />
        </button>

        <h1 className="text-2xl sm:text-3xl font-black text-[#1e40af] tracking-tight">
          {title}
        </h1>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 font-sans text-xs">
        ...
        <div className="hidden md:flex items-center gap-2 text-slate-600 font-medium">
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none bg-white border border-[#e5e9f2] hover:border-slate-300 text-slate-800 font-semibold py-1.5 pl-3 pr-8 rounded-xl cursor-pointer shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20"
            >
        ...
        <div className="hidden lg:flex items-center gap-2 text-slate-600 font-medium">
          ...
          <div className="segmented-control">
            <button
              ...
              className={clsx(
                'segmented-item',
                selectedAggregate === 'node' ? 'segmented-item-active' : 'hover:text-slate-900'
              )}
            >
              Node
            </button>
            <button
              ...
              className={clsx(
                'segmented-item',
                selectedAggregate === 'selected' ? 'segmented-item-active' : 'hover:text-slate-900'
              )}
            >
              Selected nodes
            </button>
        ...
        {/* Date Range Pill */}
        <div className="flex items-center gap-2 bg-white border border-[#e5e9f2] px-3.5 py-1.5 rounded-xl font-medium text-slate-700 shadow-sm">
          <span>01/01/2026 - 28/08/2026</span>
          <Calendar className="w-3.5 h-3.5 text-slate-400" />
        </div>

        {/* Mode Selector Pill */}
        <div className="flex items-center bg-white border border-[#e5e9f2] p-0.5 rounded-xl shadow-sm">
          <button
            onClick={() => setMode('DEMO')}
            className={clsx(
              'px-2.5 py-1 rounded-lg text-[11px] font-bold transition-all',
              mode === 'DEMO' ? 'bg-[#2563eb] text-white shadow-xs' : 'text-slate-500 hover:text-slate-800'
            )}
          >
            Lab Sim
          </button>
          <button
            ...
            className={clsx(
              'px-2.5 py-1 rounded-lg text-[11px] font-bold transition-all',
              mode === 'HARDWARE' ? 'bg-[#ef4444] text-white shadow-xs' : 'text-slate-500 hover:text-slate-800'
            )}
          >
            ESP32
          </button>
